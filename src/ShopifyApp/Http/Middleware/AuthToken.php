...
class AuthToken
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        $parts = explode('.', $token);
        $body = json_decode(Helpers::base64url_decode($parts[1]));
        ...
        if ($body->aud !== Helpers::getShopifyConfig('api_key')) {
            throw new HttpException('Invalid token', 400);
        }
    ...
    private function checkSignature($token)
    {
        ...
        $secret = Helpers::getShopifyConfig('api_secret', $shop);
        $hmac = hash_hmac('sha256', $check, $secret, true);
        $encoded = Helpers::base64url_encode($hmac);
